...
Usage:
    python run.py                          # run all experiments, all methods
    python run.py --exp 1                  # run only Exp 1
    python run.py --exp 1 2                # run Exp 1 and Exp 2
    python run.py --seeds 42 123           # override seeds
    python run.py --exp 1 --method pareto  # run only Pareto Q in Exp 1
    python run.py --exp 1 2 --method dqn pareto  # multiple methods
"""
...
def main():
    ...
    methods = args.method  # None means all methods
    ...
    for idx in sorted(args.exp):
        ...
        mod.run(seeds=seeds, methods=methods)
