"""
plot_result_new.py  —  visualise results with Envelope Q replacing Pareto Q.

Usage:
    python plot_result_new.py --exp 0        # training curve for Exp 0
    python plot_result_new.py --exp 1        # two-obj results (DQN, Tabular, Envelope)
    python plot_result_new.py --exp 2        # three-obj results
    python plot_result_new.py --exp 3        # generalisation gap
    python plot_result_new.py                # all of the above
...
# ── Shared method style registry ─────────────────────────────────────────────
# Colors sampled from reference figure (matched to Proposed/RLBR/RSPAR lines):
#   Cyan   #17BECF  →  Cond-DQN  (main contribution, matches "Proposed")
#   Orange #FF7F0E  →  Tabular+GIP  (baseline 1, matches "RLBR")
#   Purple #CC00CC  →  Envelope Q  (baseline 2, matches "RSPAR")
METHODS = {
    "dqn":      {"label": "Cond-DQN",    "color": "#17BECF", "marker": "o"},
    "tabular":  {"label": "Tabular+GIP", "color": "#FF7F0E", "marker": "s"},
    "envelope": {"label": "Envelope Q",  "color": "#CC00CC", "marker": "D"},
}
...
def plot_exp0():
    ...
    std      = gaussian_filter1d(smoothed.std(axis=0, ddof=1), sigma=20)
...
def plot_exp1():
    ...
    for mname, minfo in METHODS.items():
        ...
        for seed_curve in smoothed:
            axes[0].plot(np.arange(len(seed_curve)), seed_curve,
                         color=minfo["color"], linewidth=0.5, alpha=0.25)
...
def plot_exp4():
    """
    Exp 4 — Sensitivity analysis.

    Three separate figures, one per method:
        exp4_sensitivity_dqn.png      — HV vs learning rate
        exp4_sensitivity_tabular.png  — HV vs grid size
        exp4_sensitivity_envelope.png — HV vs grid size
    """
    exp_dir = os.path.join(RESULTS_DIR, "exp4")

    # ── DQN: lr sweep ─────────────────────────────────────────────────────────
    dqn_lrs    = [1e-2, 1e-3, 1e-4]
    dqn_labels = ["1e-2", "1e-3", "1e-4"]
    dqn_means, dqn_cis = [], []
    for lr in dqn_lrs:
        path = os.path.join(exp_dir, f"dqn_lr{lr}", "summary.npz")
        if os.path.exists(path):
            d = np.load(path)
            dqn_means.append(float(d["hv_mean"]))
            dqn_cis.append(float(d["hv_ci95"]))
        else:
            dqn_means.append(np.nan)
            dqn_cis.append(0.0)

    if any(not np.isnan(m) for m in dqn_means):
        fig, ax = plt.subplots(figsize=(5, 4))
        color = METHODS["dqn"]["color"]
        xs = np.arange(len(dqn_labels))
        ax.bar(xs, dqn_means, color=color, width=0.5, alpha=0.88,
               yerr=dqn_cis, capsize=6,
               error_kw={"linewidth": 1.3, "ecolor": "#374151"})
        for x, mean in zip(xs, dqn_means):
            if not np.isnan(mean):
                ax.text(x, mean * 1.01, f"{mean:,.0f}", ha="center",
                        va="bottom", fontsize=8)
        ax.set_xticks(xs)
        ax.set_xticklabels([f"lr = {l}" for l in dqn_labels])
        ax.set_ylabel("Hypervolume indicator  (mean ± 95 % CI)")
        ax.set_title("Exp 4 — DQN: HV vs learning rate\n"
                     "(Cond-DQN, 2-obj DST, 5 seeds)")
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{v:,.0f}"))
        ax.grid(axis="y")
        fig.tight_layout()
        _savefig("exp4_sensitivity_dqn.png")

    # ── Tabular GIP: grid size sweep ──────────────────────────────────────────
    grid_sizes  = [5, 11, 21]
    tab_means, tab_cis = [], []
    for g in grid_sizes:
        path = os.path.join(exp_dir, f"tabular_grid{g}", "summary.npz")
        if os.path.exists(path):
            d = np.load(path)
            tab_means.append(float(d["hv_mean"]))
            tab_cis.append(float(d["hv_ci95"]))
        else:
            tab_means.append(np.nan)
            tab_cis.append(0.0)

    if any(not np.isnan(m) for m in tab_means):
        fig, ax = plt.subplots(figsize=(5, 4))
        color = METHODS["tabular"]["color"]
        xs = np.arange(len(grid_sizes))
        ax.bar(xs, tab_means, color=color, width=0.5, alpha=0.88,
               yerr=tab_cis, capsize=6,
               error_kw={"linewidth": 1.3, "ecolor": "#374151"})
        for x, mean in zip(xs, tab_means):
            if not np.isnan(mean):
                ax.text(x, mean * 1.01, f"{mean:,.0f}", ha="center",
                        va="bottom", fontsize=8)
        ax.set_xticks(xs)
        ax.set_xticklabels([f"grid = {g}" for g in grid_sizes])
        ax.set_ylabel("Hypervolume indicator  (mean ± 95 % CI)")
        ax.set_title("Exp 4 — Tabular+GIP: HV vs grid size\n"
                     "(2-obj DST, 5 seeds)")
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{v:,.0f}"))
        ax.grid(axis="y")
        fig.tight_layout()
        _savefig("exp4_sensitivity_tabular.png")

    # ── Envelope Q: grid size sweep ───────────────────────────────────────────
    env_means, env_cis = [], []
    for g in grid_sizes:
        path = os.path.join(exp_dir, f"envelope_grid{g}", "summary.npz")
        if os.path.exists(path):
            d = np.load(path)
            env_means.append(float(d["hv_mean"]))
            env_cis.append(float(d["hv_ci95"]))
        else:
            env_means.append(np.nan)
            env_cis.append(0.0)

    if any(not np.isnan(m) for m in env_means):
        fig, ax = plt.subplots(figsize=(5, 4))
        color = METHODS["envelope"]["color"]
        xs = np.arange(len(grid_sizes))
        ax.bar(xs, env_means, color=color, width=0.5, alpha=0.88,
               yerr=env_cis, capsize=6,
               error_kw={"linewidth": 1.3, "ecolor": "#374151"})
        for x, mean in zip(xs, env_means):
            if not np.isnan(mean):
                ax.text(x, mean * 1.01, f"{mean:,.0f}", ha="center",
                        va="bottom", fontsize=8)
        ax.set_xticks(xs)
        ax.set_xticklabels([f"grid = {g}" for g in grid_sizes])
        ax.set_ylabel("Hypervolume indicator  (mean ± 95 % CI)")
        ax.set_title("Exp 4 — Envelope Q: HV vs grid size\n"
                     "(2-obj DST, 5 seeds)")
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{v:,.0f}"))
        ax.grid(axis="y")
        fig.tight_layout()
        _savefig("exp4_sensitivity_envelope.png")
...
def main():
    ...
    dispatch = {0: plot_exp0, 1: plot_exp1, 2: plot_exp2, 3: plot_exp3, 4: plot_exp4}
