"""
Experiment 2 — Three-objective scalability test.

Same as Exp 1 but with the fuel-cost wrapper appended, making the reward
3-dimensional: [treasure, time_penalty, fuel].

Training budget is increased to 8,000 episodes as specified in the thesis.
The 3-simplex evaluation grid has step 0.2 → 21 points.

Results saved under results/exp2/<method>/.
"""
...
from evaluation.metrics import (
    evaluate_agent, hypervolume, eval_beta_grid_3obj
)
...
RESULT_DIR = os.path.join(os.path.dirname(__file__), "..", "results", "exp2")
EVAL_BETAS = eval_beta_grid_3obj(step=0.2)

# Reference point below worst achievable in all three objectives
HV_REF = np.array([-1.0, -210.0, -22.0])


def _make_env():
    base = mo_gym.make("deep-sea-treasure-concave-v0", float_state=True)
    return DSTFuelWrapper(base)
...
def _run_method(name, make_agent_fn, train_fn, seeds):
    ...
    for seed in seeds:
        ...
        train_returns = train_fn(agent, env, N_EPISODES, n_obj=3, seed=seed)
        env.close()
        ...
        all_hvs.append(hv)
        print(f"  [{name}] seed {seed}: HV={hv:.4f}  mean_scalar={np.mean(scalars):.3f}")

    all_hvs = np.array(all_hvs)
    ci = stats.t.ppf(0.975, df=4) * all_hvs.std(ddof=1) / np.sqrt(len(seeds))

    np.savez(
        os.path.join(method_dir, "summary.npz"),
        train_returns=np.array(all_train),
        eval_scalars=np.array(all_scalars),
        hvs=all_hvs,
    ...
    print(f"  [{name}] HV: {all_hvs.mean():.4f} ± {all_hvs.std(ddof=1):.4f}  (95% CI ±{ci:.4f})")
...
def run(seeds=SEEDS, methods=None):
    ...
    if "envelope" in active:
        results["envelope"] = _run_method(
            "envelope",
            lambda: EnvelopeQAgent(n_states=N_STATES, n_actions=4, n_obj=3,
                                   n_grid_points=11, gamma=0.99, lr=0.1,
                                   eps_start=1.0, eps_end=0.05),
            train_tabular, seeds,
        )
